...
export default async function ReviewPage({
  ...
  const { folder, source } = await searchParams;
  ...
  if (folder) {
    const { data: links } = await supabase
      ...
      .select("flashcard_id")
      .eq("folder_id", folder);
    folderCardIds = (links ?? []).map((l) => l.flashcard_id);
  }

  // フォルダ/出典の絞り込みを共通適用
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  function scopeQuery(q: any) {
    if (folderCardIds) {
      return folderCardIds.length > 0 ? q.in("id", folderCardIds) : q.in("id", ["00000000-0000-0000-0000-000000000000"]);
    }
    if (source) {
      return source === "その他" ? q.is("source_title", null) : q.eq("source_title", source);
    }
    return q;
  }
  ...
  const { data: reviewCards } = await scopeQuery(
    supabase
      .from("flashcards")
      .select("*")
      .eq("user_id", user.id)
